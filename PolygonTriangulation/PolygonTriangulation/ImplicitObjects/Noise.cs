...
namespace PolygonTriangulation.ImplicitObjects {
	public class Noise {
		/// <summary>
		/// Returns a noise value for the given vector.
		/// </summary>
		public static float PerlinNoise(Vec3 v) {
			return (float) PerlinNoise(v.X, v.Y, v.Z);
		}
		// Translated from http://cs.nyu.edu/~perlin/noise/
		// Optimized version of Perlin Noise, from Ken Perlin himself.
		...
		public static double PerlinNoise(double x, double y, double z) {
			...
			var result = lerp(w, lerp(v, lerp(u, grad(p[AA], x, y, z),          // AND ADD
